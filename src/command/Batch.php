...
class Batch extends Command
{
    ...
    {
        $resource = ResourceService::getData(intval($this->queue->data['resource_id']),['ids'=>$this->queue->data['vod_ids'],'ac'=>'detail']);
        $types = CinemaTypeItem::mk()->column('type_id,type_pid','resource_type_id');
        ...
        foreach ($resource['list'] as &$model) try {
            ...
            $this->app->db->transaction(function () use ($model, $type,$region_id,$theme) {
                $video_id = ResourceService::getVideoId($model, $type,$region_id,$theme);
            ...
            $this->queue->message($total, $count, sprintf('结束采集【 %s 】资源详情', $model['vod_name']));
